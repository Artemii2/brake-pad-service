import { AutoProcessor, AutoTokenizer, RawImage, SiglipTextModel, SiglipVisionModel, env } from "@huggingface/transformers";
...
class SiglipService {
  ...
  static async init(progressCallback?: (data: unknown) => void) {
    ...
    this.processor = await AutoProcessor.from_pretrained(MODEL_ID, { progress_callback: progressCallback });
  ...
  try {
    if (type === "init") {
      ...
      for (let i = 0; i < items.length; i += 1) {
        ...
        embeddings[items[i].id] = Array.from(textOutput.data.slice(start, start + EMBEDDING_SIZE));
      }
    ...
    if (type === "image") {
      ...
      const { pooler_output: imageOutput } = await SiglipService.visionModel!(imageInputs);

      self.postMessage({ type: "image_embedding_ready", data: Array.from(imageOutput.data) });
